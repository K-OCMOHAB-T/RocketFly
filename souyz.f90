program jopa
    ...
    write(11,'(a15,1X,a15,1X,a15,1X,a15,1X,a15)') 'time', 'alfa', 'F/mass', 'R/mass', 'g'
    ...
    type(PDUtype) pdu
    double precision curvate, cosp
    
        curvate = atan(x/(y+R))
        
        cosp = sqrt(ux*ux + uy*uy)
        if (cosp .ne. 0.0d+0) then 
            cosp = ux/cosp
        else
            cosp = 0.0d+0
        endif
        ...
        Ax = (Fforce(t, pdu%P) * cos(angle(t) - curvate) - Rforce(t, ux, uy, pdu) * cosp)/mass(t) - grav(h)*sin(curvate)
    ...
    double precision curvate, sinp
        ...
        Ay = (Fforce(t, pdu%P) * sin(angle(t) - curvate) - Rforce(t, ux, uy, pdu) * sinp)/mass(t) - grav(h)*cos(curvate)
    ...
    double precision curvate, h, psi
        ...
        write(11,'(f15.5,1X,f15.5,1X,f15.5,1X,f15.5,1X,f15.5)') t, (angle(t) - curvate - psi)*radtograd,Fforce(t, pdu%P), Rforce(t, ux, uy, pdu), grav(h)*mass(t)
